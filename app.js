...
function getTileUrl(z, x, y) {
  if (state.onlineTiles) return `https://tile.openstreetmap.org/${z}/${x}/${y}.png`;
...
function loadTile(z, x, y) {
  ...
  const key = `${state.onlineTiles ? 'online' : 'offline'}:${z}/${wrappedX}/${y}`;
...
function setupEvents() {
  ...
  $('tileBtn').addEventListener('click', () => {
    state.onlineTiles = !state.onlineTiles;
    $('tileBtn').textContent = state.onlineTiles ? 'Online kaart' : 'Offline tiles';
    draw();
  });
...
async function init() {
  ...
  updateGpsUi();
  updateRoutePanel();
  setStatus('Leeg netwerk', 'Geen snelheidsgebieden of standaard routes geladen. Open de editor om je eigen vaarwegen te tekenen.', 'good');
